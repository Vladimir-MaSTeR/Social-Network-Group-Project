...
class PersonControllerIntegrationTest {
  ...
  void deleteCurrentUser() {
    HttpHeaders headers = getHeaders();
    HttpEntity<String> entity = new HttpEntity<>("body", headers);
    ResponseEntity<ResponseApi<? extends MessageSendRequestBodyApi>> response =
        getExchangeMessage("/users/me", HttpMethod.DELETE, entity);

    Assertions.assertEquals(HttpStatus.OK, response.getStatusCode());
    Assertions.assertEquals("Your profile has been successfully deleted.",
        response.getBody().getError());
    Assertions.assertEquals("Ok", response.getBody().getData().getMessageText());
  }
...
//        Don't know how to fix this shit with id nicer
  ...
  void searchUser() {
    ...
    ResponseEntity<PersonListApi> response = restTemplate
        .exchange("/users/search?first_name=Johnny&last_name=Sins&age_from=18&age_to=40&town"
                + "=New York&offset=0&itemPerPage=5", HttpMethod.GET, entity,
  ...
  private ResponseEntity<ResponseApi<? extends MessageSendRequestBodyApi>> getExchangeMessage
      (String url, HttpMethod HttpMethod, HttpEntity<String> entity) {
    return restTemplate.
        exchange(url, HttpMethod, entity,
            new ParameterizedTypeReference<ResponseApi<? extends MessageSendRequestBodyApi>>() {
              @Override
              public Type getType() {
                return super.getType();
              }
